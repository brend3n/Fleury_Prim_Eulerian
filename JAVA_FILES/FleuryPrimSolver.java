...
// HashSet<Integer> V_S = new HashSet<Integer>();
	...
	*/
	...
	private void update_lists_prims(int vertex){
	...
	private void check_answer(ArrayList<Integer> circuit){
		ArrayList<String> list = new ArrayList<String>();
		int len = 0;
		for(int i = 0; i < circuit.size() - 1; i++){
			list.add(circuit.get(i).toString() + "->"+circuit.get(i+1).toString());
			len++;
		}
	...
	public void Fleuryify(int start){
		...
		// }
		// check_answer(circuit);
		writeArrayListToFile("output.txt", circuit);
		System.out.println("Wrote circuit to: output.txt");
